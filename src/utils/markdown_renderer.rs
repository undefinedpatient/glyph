...
impl<'a> MarkdownTable<'a> {
    ...
    fn next_column(&mut self) {
        self.position.1 = self
            ...
            .1
            .saturating_add(1)
            .clamp(0, self.size().1.saturating_sub(1));
    }
        ...
        for (r_idx, row) in self.data.iter().enumerate() {
            ...
            if r_idx != 0 {
                ...
                for (c_idx, cell) in row.iter().enumerate() {
                    ...
                    let mut cell_width = cell_sizes.0[c_idx];
                    line.push_span(Span::raw(
                        { if r_idx == 1 { "━" } else { "─" } }.repeat(cell_width),
                    ));
                    if c_idx == row.len() - 1 {
                        line.push_span(Span::raw(if r_idx == 1 { "┩" } else { "┤" }));
                        break;
            ...
            for (c_idx, cells) in row.iter().enumerate() {
                ...
                let num_of_empty_padding: usize =
                    cell_sizes.0[c_idx].saturating_sub(individual_cell_width);
                ...
                let (left_padding, right_padding) = match self.alignments[c_idx] {
                    ...
                    Alignment::Center => (
                        " ".repeat(num_of_empty_padding / 2),
                        " ".repeat(num_of_empty_padding.div_ceil(2)),
                    ),
                    Alignment::Right => (" ".repeat(num_of_empty_padding), "".to_string()),
                };
...
impl<'a> MarkdownRenderer<'a> {
    ...
    fn render_buffer(&mut self, buffer: &mut Buffer) {
        if self.quote_state.level != 0 {
            self.spans_buffer
                .insert(0, Span::from("░ ".repeat(self.quote_state.level as usize)));
        }
            ...
            match level {
                HeadingLevel::H1 => {
                    let text = BigText::builder()
                        .lines([Line::from(self.spans_buffer.clone())])
                        .pixel_size(PixelSize::Full)
                        .build();
                    text.render(
                        line_area
                            .resize(Size::new(line_area.width, 8))
                ...
                HeadingLevel::H3 => {
                    ...
                    text.render(
                        line_area
                            .resize(Size::new(line_area.width, 4))
                            .intersection(self.area),
                        buffer,
                    );
                ...
                HeadingLevel::H5 => {
                    ...
                    text.render(
                        line_area
                            .resize(Size::new(line_area.width, 3))
                            .intersection(self.area),
                        buffer,
                    );
                ...
                HeadingLevel::H6 => {
                    ...
                    text.render(
                        line_area
                            .resize(Size::new(line_area.width, 2))
                            .intersection(self.area),
                        buffer,
                    );
    ...
    pub fn render(mut self, str: &'a str, buffer: &mut Buffer) {
        ...
        for event in parser {
            match event {
                    ...
                    match tag {
                        ...
                        Tag::Item => {
                            ...
                            if let Some(list_state) = self.list_state_stack.last_mut() {
                                ...
                                // Add marker
                                self.spans_buffer.push(Span::raw(format!(
                                    "{:<0}{1} ",
                                    indent,
                                    list_state.get_prefix()
                                )));
                ...
                Event::Rule => {
                    self.spans_buffer = vec![
                        Span::from(format!(
                            "{:—<width$}",
                            "",
                            width = self.area.width.saturating_sub(1) as usize
                        ))
                        .dark_gray(),
                    ];
                    self.render_buffer(buffer);
                ...
                Event::Code(text) => {
                    self.spans_buffer
                        .push(Span::raw(text).bg(self.theme.surface_low_highlight()));
                }
                Event::Text(text) => {
                    ...
                    if self.is_in_table {
                        self.table
                            .push_span(Span::styled(text, self.text_style.build(self.theme)))
                            .unwrap();
                        continue;
                    }
                    self.spans_buffer
                        .push(Span::styled(text, self.text_style.build(self.theme)));
                }
                ...
                Event::End(tag) => {
                    match tag {
                        ...
                        TagEnd::CodeBlock => {
                            ...
                            let text: Text =
                                Text::from(self.code_lines).bg(self.theme.surface_low_highlight());
                            let text_height: usize = text.height();
                            let text_width: usize = text.width();
                            if let Some(line_area) = self.rows_area.get(self.render_row_index) {
                                let code_block_frame: Block = Block::bordered()
                                    .border_style(Style::default().dim())
                                    .border_type(BorderType::LightDoubleDashed);
                                let code_block_area: Rect = line_area.resize(Size::new(
                                    text_width as u16 + 2,
