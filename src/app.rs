...
impl<T: Interactable + Drawable> Element for T {
    ...
    fn as_element_ref(&self) -> &dyn Element { self }
    fn as_element_mut(&mut self) -> &mut dyn Element { self }
}
impl<T: Interactable + Drawable + Focusable> Container for T{
    ...
    fn as_view_ref(&self) -> &dyn Container { self }
    fn as_view_mut(&mut self) -> &mut dyn Container { self }
}
