...
impl Drawable for EntrancePage {
    fn render(&self, area: Rect, buf: &mut Buffer, draw_flag: DrawFlag)
    {
        ...
        let block: Block = match draw_flag {
            ...
            DrawFlag::HIGHLIGHTING => Block::bordered().borders(Borders::ALL).border_type(BorderType::Double),
            DrawFlag::FOCUSED => Block::bordered().borders(Borders::ALL).border_type(BorderType::Double),
        };
        /*
           Title
         */
...
impl Drawable for CreateGlyphPage {
    fn render(&self, area: Rect, buf: &mut Buffer, draw_flag: DrawFlag) {
        ...
        let button_areas = Layout::horizontal([
            Constraint::Max(20),
            Constraint::Max(20),
        ]).flex(Flex::Center).split(chunks[1]);
