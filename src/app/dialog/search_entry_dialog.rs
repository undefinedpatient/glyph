...
pub struct SearchEntryDialog {
    ...
    on_submit:
        Option<Box<dyn Fn(Option<&mut dyn Any>, Option<&mut dyn Any>) -> Result<Vec<Command>>>>,
}
impl SearchEntryDialog {
    pub fn new(label: &str, local_entry_state: Rc<RefCell<LocalEntryState>>) -> SearchEntryDialog {
        let entries: Vec<(i64, String)> = (*local_entry_state.borrow().entries)
            .iter()
        ...
        Self {
            ...
            text_field: TextField::new("", "", Box::new(|_value| true)).on_update(Box::new(
                |parent_state, state| {
                    let mut _parent_state = parent_state
                        .unwrap()
                    ...
                    _parent_state.hovered_index = _parent_state
                        .hovered_index
                        .clamp(0, ava_count)
                        .saturating_sub(1);
                    Ok(vec![])
    ...
    pub fn on_submit(
        mut self,
        on_submit: Box<dyn Fn(Option<&mut dyn Any>, Option<&mut dyn Any>) -> Result<Vec<Command>>>,
    ) -> Self {
...
impl Drawable for SearchEntryDialog {
    fn render(&self, frame: &mut Frame, area: Rect, draw_flag: DrawFlag, theme: &dyn Theme) {
        ...
        self.text_field
            .render(frame, text_field_area, draw_flag, theme);
            ...
            KeyEventKind::Press => {
                ...
                if key.modifiers.contains(KeyModifiers::CONTROL) {
                    if let KeyCode::Char('n') = key.code {
                        self.state.hovered_index = cycle_offset(
                            self.state.hovered_index as u16,
                            1,
                            self.state
                                .entries_name
                                .iter()
                                .filter(|(_name, ava)| *ava)
                                .count() as u16,
                        ) as usize;
                    }
