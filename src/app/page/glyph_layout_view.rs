...
impl Drawable for GlyphLayoutView {
    fn render(&self, frame: &mut Frame, area: Rect, _draw_flag: DrawFlag, theme: &dyn Theme) {
        /*
           Container Frame
        */
        let edit_areas = ratatui::layout::Layout::horizontal([
...
impl GlyphLayoutOverview {
    ...
    pub(crate) fn cycle_layout_hover(&mut self, offset: i16) {
        ...
        let len: usize = ref_layout
            .get_layout_at_ref(&select_coordinate)
            .unwrap()
            .sub_layouts
            .len();
        drop(state);
        if let Some(hover_index) = self.state.hovered_index {
            self.state.hovered_index =
                Some(cycle_offset(hover_index as u16, offset, len as u16) as usize);
        } else if len > 0 {
        ...
        match layout.details.size_mode {
            ...
            SizeMode::Length => {
                ...
                scroll_view.render(
                    area,
                    frame.buffer_mut(),
                    &mut *self.state.scroll_state.borrow_mut(),
                );
            }
        }
    }
                    ...
                    match c {
                        ...
                        'A' => {
                            ...
                            let layout: &mut Layout =
                                &mut state.get_entry_mut(&eid).unwrap().layout;
                            layout.insert_sublayout_under(Layout::new(""), &target_coor);
                            return Ok(vec![GlyphCommand(SetEntryUnsavedState(eid, true))]);
                        }
                        'D' => {
                            return Ok(vec![PageCommand(PushDialog(
                                SearchEntryDialog::new(
                                    "Clone Active Entry Layout to",
                                    self.state.entry_state.clone(),
                                )
                                .on_submit(Box::new(|parent_state, state| {
                                    let _parent_state: &mut GlyphPageState = parent_state
                                        .unwrap()
                                        .downcast_mut::<GlyphPageState>()
                                        .unwrap();
                                    let _state: &mut SearchEntryDialogState = state
                                        .unwrap()
                                        .downcast_mut::<SearchEntryDialogState>()
                                        .unwrap();
                                    if _state.entries_name.is_empty() {
                                        return Ok(vec![]);
                                    }
                                    let filtered_eids = _state
                                        .entries_name
                                        .iter()
                                        .filter_map(|((eid, _name), ava)| {
                                            if *ava {
                                                return Some(*eid);
                                            }
                                            None
                                        ...
                                        .collect::<Vec<i64>>();
                                    let selected_eid =
                                        filtered_eids.get(_state.hovered_index).unwrap();
                                    let mut _entry_state = _state.local_entry_state.borrow_mut();
                                    let active_entry_layout: Layout =
                                        _entry_state.get_active_entry_ref().unwrap().layout.clone();
                                    let entry = _entry_state
                                        .entries
                                        .iter_mut()
                                        .find(|(eid, entry)| *eid == *selected_eid)
                                        .unwrap();
                        ...
                        '+' => {
                            ...
                            let sublayout: &mut Layout =
                                layout.get_layout_at_mut(&target_coor).unwrap();
                            if sublayout.section_index.is_none() {
                                sublayout.section_index = Some(0);
                            } else {
                                sublayout.section_index =
                                    Some(sublayout.section_index.unwrap() + 1);
                            }
                            return Ok(vec![GlyphCommand(SetEntryUnsavedState(eid, true))]);
                        }
                        '-' => {
...
fn evaluate_layout(
    ...
    let mut block: Block = Block::bordered()
        .title(layout.label.as_str())
        .padding(Padding::uniform(layout.details.padding))
    ...
    let constraints: Vec<Constraint> = layout
        .sub_layouts
        .iter()
        .map(|sub| match sub.details.size_mode {
            SizeMode::Flex => Constraint::Fill(sub.details.flex),
            SizeMode::Length => Constraint::Length(sub.details.length),
        })
        .collect();

    let intermediate_area: Rect = Block::default()
        .padding(Padding::uniform(if depth == 0 {
            0
        } else {
    ...
    let sub_areas = match layout.details.orientation {
        LayoutOrientation::Vertical => {
...
impl GlyphLayoutEditView {
    pub fn new(
        ...
        Self {
            containers: vec![
                TextField::new("Label", "", Box::new(|_value| true))
                    .on_exit(Box::new(|parent_state, state| {
                        let _parent_state = parent_state
                            .unwrap()
                            .downcast_mut::<GlyphLayoutEditState>()
                            .unwrap();
                        ...
                        let mut local_entry_state: RefMut<LocalEntryState> =
                            _parent_state.local_entry_state_mut().unwrap();
            ...
            components: vec![
                OptionMenu::new(vec![("Flex".to_string(), 0), ("Length".to_string(), 1)], 0)
                    .on_interact(Box::new(|parent_state, state| {
                        let _parent_state = parent_state
                            .unwrap()
                            .downcast_mut::<GlyphLayoutEditState>()
                            .unwrap();
                        ...
                        let selected_item_value: u8 =
                            _state.options[selected_item_index as usize].1;
                        let parsed_selected_item = match selected_item_value {
                            0 => SizeMode::Flex,
                            1 => SizeMode::Length,
                            _ => {
                                return Err(Report::msg(
                                    "Impossible to have another value for size mode.",
                                ));
                            }
                        };
                ...
                .on_interact(Box::new(|parent_state, state| {
                    ...
                    let eid: i64 = local_entry_state.active_entry_id.unwrap();
                    let entry: &mut Entry = local_entry_state.get_entry_mut(&eid).unwrap();
                    let sublayout_to_update: &mut Layout =
                        entry.layout.get_layout_at_mut(&coor).unwrap();
                ...
                Button::new("Revert All")
                    .on_interact(Box::new(|parent_state| {
                        let _parent_state: &mut GlyphLayoutEditState = parent_state
                            .unwrap()
                            .downcast_mut::<GlyphLayoutEditState>()
                            .unwrap();
                        let eid: i64 = _parent_state
                            .local_entry_state_mut()
                            .unwrap()
                            .active_entry_id
                            .unwrap();
                        _parent_state
    ...
    pub fn refresh_layout_edit_panel(&mut self) {
        ...
        (*self.containers[0])
            ...
            .unwrap()
            .replace(root_layout_label);
        // Size
        (*self.components[0])
            .as_any_mut()
            .downcast_mut::<OptionMenu>()
            .unwrap()
            .replace(root_layout_size_mode);
        // Border Type
        (*self.components[1])
            .as_any_mut()
            .downcast_mut::<OptionMenu>()
            .unwrap()
            .replace(root_layout_border_mode);
        // Length
        (*self.containers[1])
            .as_any_mut()
            .downcast_mut::<NumberField>()
            .unwrap()
            .replace(root_layout_length as i16);
        // Flex
        (*self.containers[2])
            .as_any_mut()
            .downcast_mut::<NumberField>()
            .unwrap()
            .replace(root_layout_flex as i16);
        // Padding
        (*self.containers[3])
            .as_any_mut()
            .downcast_mut::<NumberField>()
            .unwrap()
            .replace(root_layout_padding as i16);
        // Margin
        (*self.containers[4])
            .as_any_mut()
            .downcast_mut::<NumberField>()
            .unwrap()
            .replace(root_layout_margin as i16);
    }
}
...
impl Drawable for GlyphLayoutEditView {
    fn render(&self, frame: &mut Frame, area: Rect, draw_flag: DrawFlag, theme: &dyn Theme) {
        ...
        */
        ...
        let chunks =
            ratatui::layout::Layout::vertical([Constraint::Percentage(100), Constraint::Length(1)])
                .split(inner_area);
        let field_areas =
            ratatui::layout::Layout::vertical([Constraint::Length(3)].repeat(7)).split(chunks[0]);
        ...
        // Flex Field
        self.containers[2].render(
            frame,
            field_areas[4],
            get_draw_flag(
                self.state.hovered_index,
                4,
                Some(self.containers[2].is_focused()),
            ),
            theme,
        );
        // Padding Field
        self.containers[3].render(
            frame,
            field_areas[5],
            get_draw_flag(
                self.state.hovered_index,
                5,
                Some(self.containers[3].is_focused()),
            ),
            theme,
        );
        // Margin Field
        self.containers[4].render(
            frame,
            field_areas[6],
            get_draw_flag(
                self.state.hovered_index,
                6,
                Some(self.containers[4].is_focused()),
            ),
            theme,
        );

        // Revert Button
        self.components[2].render(
            frame,
            chunks[1],
